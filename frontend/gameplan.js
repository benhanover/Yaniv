...
createGame(gameControl) {
    while (gameControl.players.length > 1) {
        round(players);
        gameControlUpdate(players)
        //   -optional removePlayers();
        scoreUpdate(players);
        // 1 function that hids the welcome page
        // 2 method in player that reset the score and sums it in sum property
        // 3 build scoreUpDate to be array of objects each object resamble a round in the game
